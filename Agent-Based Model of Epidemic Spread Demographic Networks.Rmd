...
graph <- graph_from_adjacency_matrix(bioTransition, mode = "directed", weighted = TRUE)

# Extract edge weights (transition probabilities)
edge_weights <- E(graph)$weight

# Scale edge weights for visualization (adjust the multiplier as needed)
scaled_weights <- edge_weights * 10  # Adjust multiplier to make lines more visible

# Define layout
layout <- layout_with_fr(graph)

# Plot graph with scaled edge thickness
plot(
  graph, 
  layout = layout,
  edge.width = scaled_weights,  # Set edge width based on probability
  edge.arrow.size = 0.7,  # Adjust arrow size
  vertex.color = "lightcoral",
  vertex.size = 15,
  vertex.label = STATELABELS,
  edge.label = round(edge_weights, 2)  # Show transition probabilities
)
